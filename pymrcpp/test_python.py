...
def test_evalf_pelm():
    assert isclose(phi_tree_pois.evalf(0, 0 , 0), phi_exact(0, 0, 0), rel_tol = prec)

def test_add():
    mr.add(prec/10, add_tree, 1.0, phi_tree, -1, phi_tree_pois)
    print(add_tree.evalf(0,0,0))
    assert isclose(add_tree.evalf(0, 0, 0), 0.0, abs_tol = prec*10)


def test_multiply():
    mr.multiply(prec, mult_tree, 1, phi_tree, phi_tree_pois)
    assert isclose(mult_tree.evalf(0, 0, 0), phi_exact(0, 0, 0)**2, rel_tol = prec)
