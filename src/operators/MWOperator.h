...
public:
    ...
    OperatorTreeT<T> &getComponent(int i, int d);
    const OperatorTreeT<T> &getComponent(int i, int d) const;

    std::array<OperatorTreeT<T> *, D> &operator[](int i) { return this->oper_exp[i]; }
    const std::array<OperatorTreeT<T> *, D> &operator[](int i) const { return this->oper_exp[i]; }

protected:
    int oper_root;
    int oper_reach;
    MultiResolutionAnalysis<D> MRA;
    std::vector<std::array<OperatorTreeT<T> *, D>> oper_exp;
    std::vector<std::unique_ptr<OperatorTreeT<T>>> raw_exp;
    std::vector<int> band_max;

    MultiResolutionAnalysis<2> getOperatorMRA() const;

    void initOperExp(int M);
    void assign(int i, int d, OperatorTreeT<T> *oper) { this->oper_exp[i][d] = oper; }
};
