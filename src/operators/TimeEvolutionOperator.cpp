...
TimeEvolutionOperator<D, T>::TimeEvolutionOperator(const MultiResolutionAnalysis<D> &mra, double prec, double time, int finest_scale, bool imaginary, int max_Jpower)
        : ConvolutionOperator<D, T>(mra, mra.getRootScale(), -10) // One can use ConvolutionOperator instead as well
{
 ...
 */
template <int D, typename T>
TimeEvolutionOperator<D, T>::TimeEvolutionOperator(const MultiResolutionAnalysis<D> &mra, double prec, double time, bool imaginary, int max_Jpower)
        : ConvolutionOperator<D, T>(mra, mra.getRootScale(), -10) // One can use ConvolutionOperator instead as well
{
    int oldlevel = Printer::setPrintLevel(0);
    this->setBuildPrec(prec);
 ...
 *
 */
template <int D, typename T> void TimeEvolutionOperator<D, T>::initialize(double time, bool imaginary, int max_Jpower) {
    int N = 18;

    double o_prec = this->build_prec;
    auto o_mra = this->getOperatorMRA();
    auto o_tree = std::make_unique<CornerOperatorTree<T>>(o_mra, o_prec);

    std::map<int, JpowerIntegrals *> J;
    for (int n = 0; n <= N + 1; n++) J[n] = new JpowerIntegrals(time * std::pow(4, n), n, max_Jpower);
    TimeEvolution_CrossCorrelationCalculator<T> calculator(J, this->cross_correlation, imaginary);

    OperatorAdaptor adaptor(o_prec, o_mra.getMaxScale(), true);

    mrcpp::TreeBuilder<2, T> builder;
 ...
 *
 */
template <int D, typename T> void TimeEvolutionOperator<D, T>::initialize(double time, int finest_scale, bool imaginary, int max_Jpower) {
    double o_prec = this->build_prec;
    auto o_mra = this->getOperatorMRA();

    // Setup uniform tree builder
    TreeBuilder<2, T> builder;
    SplitAdaptor<2, T> uniform(o_mra.getMaxScale(), true);
 ...
 *
 */
template <int D, typename T> void TimeEvolutionOperator<D, T>::initializeSemiUniformly(double time, bool imaginary, int max_Jpower) {
    ...
    mrcpp::TreeBuilder<2, T> builder;
    mrcpp::SplitAdaptor<2, T> uniform(o_mra.getMaxScale(), true);

    int N = 18;

    auto o_tree = std::make_unique<CornerOperatorTree<T>>(o_mra, o_prec);
    DefaultCalculator<2, T> intitial_calculator;
    ...
    mrcpp::TimeEvolution_CrossCorrelationCalculator<T> calculator(J, this->cross_correlation, imaginary);
...
template TimeEvolutionOperator<1, ComplexDouble>::TimeEvolutionOperator(const MultiResolutionAnalysis<1> &, double, double, int, bool, int);
template void TimeEvolutionOperator<1, ComplexDouble>::initialize(double, int, bool, int);
