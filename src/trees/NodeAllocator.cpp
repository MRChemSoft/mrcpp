...
template <>
NodeAllocator<2>::NodeAllocator(OperatorTreeT<double> *tree, SharedMemory<double> *mem, int coefsPerNode, int nodesPerChunk)
        : coefsPerNode(coefsPerNode)
        , maxNodesPerChunk(nodesPerChunk)
        , tree_p(tree)
        , shmem_p(mem) {
    ...
    OperatorNodeT<double> tmp;
    this->cvptr = *(char **)(&tmp);
    this->sizeOfNode = sizeof(OperatorNodeT<double>);

    MRCPP_INIT_OMP_LOCK();
}

template <>
NodeAllocator<2, ComplexDouble>::NodeAllocator(OperatorTreeT<ComplexDouble> *tree, SharedMemory<ComplexDouble> *mem, int coefsPerNode, int nodesPerChunk)
        : coefsPerNode(coefsPerNode)
        , maxNodesPerChunk(nodesPerChunk)
        , tree_p(tree)
        , shmem_p(mem) {
    this->nodeChunks.reserve(100);
    this->coefChunks.reserve(100);
    OperatorNodeT<ComplexDouble> tmp;
    this->cvptr = *(char **)(&tmp);
    this->sizeOfNode = sizeof(OperatorNodeT<ComplexDouble>);
    MRCPP_INIT_OMP_LOCK();
}

template <int D, typename T> NodeAllocator<D, T>::NodeAllocator(OperatorTreeT<T> *tree, SharedMemory<T> *mem, int coefsPerNode, int nodesPerChunk) {
    NOT_REACHED_ABORT;
}
