...
template <typename T>
OperatorTreeT<T>::OperatorTreeT(const MultiResolutionAnalysis<2> &mra, double np, const std::string &name)
        : MWTree<2, T>(mra, name)
        , normPrec(np)
        , bandWidth(nullptr)
...
template <typename T> void OperatorTreeT<T>::allocRootNodes() {
    ...
    for (int rIdx = 0; rIdx < nRoots; rIdx++) {
        ...
        new (root_p) OperatorNodeT<T>(this, rIdx);
        roots[rIdx] = root_p;
...
template <typename T> OperatorTreeT<T>::~OperatorTreeT() {
    clearOperNodeCache();
    clearBandWidth();
    this->deleteRootNodes();
}

template <typename T> void OperatorTreeT<T>::clearBandWidth() {
 ...
 *
 */
template <typename T> void OperatorTreeT<T>::calcBandWidth(double prec) {
    ...
    for (int depth = 0; depth < this->getDepth(); depth++) {
        ...
        while (not done) {
            ...
            MWNode<2, T> &node = getNode(depth, l);
 ...
 *
 */
template <typename T> bool OperatorTreeT<T>::isOutsideBand(int oTransl, int o_depth, int idx) {
 ...
 *
 */
template <typename T> void OperatorTreeT<T>::removeRoughScaleNoise(int trust_scale) {
...
template <typename T> void OperatorTreeT<T>::getMaxTranslations(VectorXi &maxTransl) {
 ...
 */
template <typename T> void OperatorTreeT<T>::setupOperNodeCache() {
    int nScales = this->nodesAtDepth.size();
    int rootScale = this->getRootScale();
    this->nodePtrStore = new OperatorNodeT<T> **[nScales];
    this->nodePtrAccess = new OperatorNodeT<T> **[nScales];
    VectorXi max_transl;
    getMaxTranslations(max_transl);
    for (int n = 0; n < nScales; n++) {
        ...
        auto **nodes = new OperatorNodeT<T> *[n_nodes];
        int j = 0;
        for (int i = n_transl; i >= 0; i--) {
            NodeIndex<2> idx(scale, {0, i});
            // Generated OperatorNodes are still OperatorNodes
            if (auto *oNode = dynamic_cast<OperatorNodeT<T> *>(&MWTree<2, T>::getNode(idx))) {
                nodes[j] = oNode;
                j++;
            } else {
                NOT_REACHED_ABORT;
...
template <typename T> void OperatorTreeT<T>::clearOperNodeCache() {
...
template <typename T> void OperatorTreeT<T>::mwTransformUp() {
 ...
 * of OperatorNorm is done using random vectors, which is non-deterministic
 * in parallel. FunctionTrees should be fine. */
template <typename T> void OperatorTreeT<T>::mwTransformDown(bool overwrite) {
    std::vector<MWNodeVector<2, T>> nodeTable;
    tree_utils::make_node_table(*this, nodeTable);
...
template <typename T> std::ostream &OperatorTreeT<T>::print(std::ostream &o) const {
    o << std::endl << "*OperatorTreeT: " << this->name << std::endl;
    return MWTree<2, T>::print(o);
} 

template class OperatorTreeT<double>;
template class OperatorTreeT<ComplexDouble>;
