...
SCENARIO("FunctionTree IO", "[tree_io], [trees]") {
    ...
    WHEN("a function with unused memory chunks is saved") {
        ...
        f_tree.saveTree("f");
        THEN("the old tree remains unchanged") {
            int nChunks = f_tree.getFunctionNodeAllocator().getNChunks();
            int nChunksUsed = f_tree.getFunctionNodeAllocator().getNChunksUsed();
        ...
        AND_WHEN("the saved function is load into a new tree") {
            ...
            THEN("the new tree has no empty chunks") {
                int nChunks = g_tree.getFunctionNodeAllocator().getNChunks();
                int nChunksUsed = g_tree.getFunctionNodeAllocator().getNChunksUsed();
